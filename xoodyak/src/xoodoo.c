...
void inverse_product_add(uint8_t *state, uint8_t output_index, uint8_t plane1_index, uint8_t plane2_index);
...
void non_linear_layer(uint8_t *state)
{
    inverse_product_add(state, 0, 1, 2); 
    inverse_product_add(state, 1, 2, 0); 
    inverse_product_add(state, 2, 0, 1); 
}
...
void inverse_product_add(uint8_t *state, uint8_t output_index, uint8_t plane1_index, uint8_t plane2_index)
{
    ...
    invert_plane(temp, GET_PLANE(state, plane1_index));
    and_planes(temp, temp, GET_PLANE(state, plane2_index));
    add_planes(GET_PLANE(state, output_index), GET_PLANE(state, output_index), temp);
}
